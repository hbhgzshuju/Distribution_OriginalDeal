...
#Data_original = pd.read_csv(u'e:\\data\\1.csv',keep_default_na = False,dtype = str,encoding = 'gb18030')
...
class Operation_Deal(object):
    ...
    def Operation_Deal(self,Data_original,Data_operation,Index_need):
        ##一次电话内容个性化操作##
        Column_operation = u'Sparated_reason'
...
def Operation_Freedom(Data_original,Data_freedom):
    
    Column_operation = u'Sparated_reason'
    if Column_operation not in Data_original.columns.tolist():
        Data_original[Column_operation] = u''
    ...
    for freedom in Levels_freedom:
        try:
            ...
            Data_original.loc[:,Column_operation] = Operation_Deal(Data_original,Data,Index_need).Operation_Summary()
        except:
            print(u'序号:' + str(freedom) + u'出错')
            
    return(Data_original[Column_operation].values)

#Data_original = pd.read_excel(u'e:\\data\\杂\\定类-个性化测试1.xlsx',sheetname = 0,keep_default_na = False)
#Type_ticket = Operation_Freedom(Data_original,Data_separated_reason)
#Data_original[u'Sparated_reason'] = Type_ticket
#Data_original.to_csv(u'e:\\data\\2.csv',encoding = 'gb18030',index = False)
